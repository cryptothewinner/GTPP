...
import { useSalesOrderList, SalesOrder } from '@/hooks/use-sales-orders';
...
export default function SalesOrdersPage() {
    ...
    const getRowId = useCallback((params: GetRowIdParams<SalesOrder>) => params.data.id, []);
    ...
    const filteredData = useMemo(() => {
        ...
        return orders.filter(o =>
            ...
            o.customer?.name1?.toLowerCase().includes(term)
        );
    }, [orders, searchTerm]);

    const columnDefs = useMemo<ColDef<SalesOrder>[]>(
        () => [
            {
                field: 'orderNumber',
                headerName: 'SİPARİŞ NO',
                width: 180,
                pinned: 'left',
                ...
                onCellClicked: (params) => {
                    if (params.data?.id) router.push(`/sales/orders/${params.data.id}`);
                },
            },
            {
                headerName: 'MÜŞTERİ',
    ...
    const onRowDoubleClicked = useCallback((event: any) => {
        if (event.data?.id) router.push(`/sales/orders/${event.data.id}`);
    }, [router]);
            ...
            <header className="bg-white border-b border-lightning-border p-4 md:px-6 shadow-sm shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h1 className="text-2xl font-bold text-slate-800 leading-tight">Satış Siparişleri</h1>
                        <p className="text-sm text-slate-500">Müşteri siparişlerinin listesi ve yönetimi</p>
                    </div>
                </div>
            </header>
